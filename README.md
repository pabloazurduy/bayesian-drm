...
## Problem Statement 
In the [Uber publication][1] they propose a common problem that is to maximize the total return $Y$ (churn) of a promotion based on a heterogeneity model. For the purpose of illustrating a different problem we are going to use a different example but with a similar objective. 
